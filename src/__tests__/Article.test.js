...
test("renders a <small> with the date of the post", () => {
  const date = new Date("December 15, 2020");
  const { getByTestId } = render(
    <Article title="Test Article" date={date} preview="This is a preview" data-testid="article" />
  );
  const small = getByTestId("article-date");
  expect(small).toBeInTheDocument();
  expect(small.tagName).toBe("SMALL");
  expect(small.textContent).toBe("12/15/2020");
});
